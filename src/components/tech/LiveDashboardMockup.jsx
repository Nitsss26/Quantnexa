...
export default function LiveDashboardMockup() {
  ...
  const [notifications, setNotifications] = useState([
    { text: 'NEW STUDENT: ADM 2026 094 verified.', time: 'Just now' },
    ...
    <div className="w-full relative glass-panel rounded-xl overflow-hidden shadow-2xl border border-cyber-border/30 bg-black/30 backdrop-blur-md font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 py-3 bg-[#050b08] border-b border-cyber-border/30">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-cyber-text tracking-widest font-mono">ERP - CORE PORTAL v2.8</span>
        </div>
          ...
          {MODULES.map((mod) => {
            ...
            return (
              <div key={mod.id} className="relative w-full">
                {/* Click Indicator for Faculty Hub */}
                {mod.id === 'faculty' && activeTab === 'admissions' && (
                  <div className="absolute -top-4 right-4 flex flex-col items-center animate-bounce pointer-events-none z-20">
                    <span className="text-[8px] font-bold text-neon-green tracking-wider bg-black/90 px-1.5 py-0.5 rounded border border-neon-green/50 shadow-[0_0_10px_rgba(34,255,102,0.3)] whitespace-nowrap">CLICK HERE</span>
                    <div className="w-0 h-0 border-l-[4px] border-r-[4px] border-t-[5px] border-transparent border-t-neon-green/80 mt-0.5"></div>
                  </div>
                )}
                <button
        ...
        <div className="flex-1 p-5 flex flex-col justify-between relative min-h-[300px]">
          ...
          {loading ? (
            <div className="flex-1 flex flex-col items-center justify-center font-mono text-xs text-neon-green gap-2">
              ...
              <span>PULLING SECURE RECORDS FROM DB...</span>
            </div>
          ) : (
            <div className="relative z-10 flex-1 flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <div className="bg-[#050b08]/80 p-3 rounded border border-cyber-border/30">
                  <span className="block text-[9px] font-mono text-cyber-text/60 tracking-wider">SYSTEM LEARNERS</span>
                  <span className="text-sm font-mono text-white font-bold block mt-0.5 tracking-wide">
                    {studentCount.toLocaleString()}
                  </span>
                </div>
                <div className="bg-[#050b08]/80 p-3 rounded border border-cyber-border/30">
                  <span className="block text-[9px] font-mono text-cyber-text/60 tracking-wider">FACULTY ACTIVE</span>
                ...
                <div className="bg-[#050b08]/80 p-3 rounded border border-cyber-border/30 col-span-2 sm:col-span-1">
                  <span className="block text-[9px] font-mono text-cyber-text/60 tracking-wider">ACTIVE NETWORK</span>
                ...
                {activeTab === 'admissions' && (
                  <div className="space-y-3 font-mono">
                    <div className="flex items-center justify-between text-xs border-b border-cyber-border/20 pb-2">
                      ...
                      <span className="text-neon-green text-[10px]">LIVE FEED</span>
                    </div>
                    <div className="space-y-2">
                      <div className="flex justify-between text-[11px] items-center p-2 rounded bg-neon-green/5 border border-neon-green/10">
                        ...
                        <span className="text-neon-emerald">DOCS OK - ENROLLED</span>
                      </div>
                      <div className="flex justify-between text-[11px] items-center p-2 rounded bg-neon-green/5 border border-neon-green/10">
                ...
                {activeTab === 'faculty' && (
                  <div className="space-y-3 font-mono">
                    <div className="flex items-center justify-between text-xs border-b border-cyber-border/20 pb-2">
                      ...
                      <span className="text-cyan-400 text-[10px]">ACTIVE LECTURES</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-[10px]">
                      <div className="p-2 border border-cyber-border/30 rounded bg-[#050b08]">
                        ...
                        <span className="text-neon-green animate-pulse text-[8px] mt-0.5 block">SYS ENGAGED</span>
                      </div>
                      <div className="p-2 border border-cyber-border/30 rounded bg-[#050b08]">
                ...
                {activeTab === 'exams' && (
                  <div className="space-y-3 font-mono">
                    <div className="flex items-center justify-between text-xs border-b border-cyber-border/20 pb-2">
                      ...
                      <span className="text-yellow-400 text-[10px]">TERM END FINALS</span>
                    </div>
                    ...
                    <div className="relative h-20 w-full mt-2">
                      ...
                      <div className="absolute top-1 left-2 text-[9px] text-neon-green/70">STUDENT PERFORMANCE INDEX</div>
                    </div>
                  </div>
                )}
                ...
                {activeTab === 'billing' && (
                  <div className="space-y-3 font-mono">
                    <div className="flex items-center justify-between text-xs border-b border-cyber-border/20 pb-2">
                      ...
                      <span className="text-purple-400 text-[10px]">RECONCILED LEDGER</span>
                    </div>
                    <div className="flex justify-between items-center text-[11px] p-2 bg-[#050b08] border border-cyber-border/30 rounded">
                      <span className="text-cyber-text">REVENUE COLLECTION</span>
                    ...
                    <div className="flex justify-between items-center text-[10px] text-cyan-400">
                      ...
                      <span className="underline cursor-pointer flex items-center gap-1">VIEW LEDGERS <ArrowRight className="w-3 h-3" /></span>
                    </div>
                  </div>
                )}
              </div>
          ...
          <div className="border-t border-cyber-border/20 pt-3 flex items-center gap-3 font-mono text-[9px] text-cyber-text/80">
            ...
            <div className="flex-1 overflow-hidden h-3.5 relative">
              <div className="absolute w-full animate-pulse truncate">
                SYS MSG - {notifications[0].text} ({notifications[0].time})
              </div>
            </div>
          </div>
        </div>
