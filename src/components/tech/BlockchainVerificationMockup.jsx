...
export default function BlockchainVerificationMockup() {
  ...
  const triggerVerification = (tamperState = false) => {
    ...
    // Reset logs
    const initialLogs = [
      'SYS - NODE RUNNER REBOOTING',
      tamperState 
        ? 'ALERT - RE-VERIFYING LEDGER WITH SECURITY OVERRIDE ENABLED' 
    ...
    const interval = setInterval(() => {
      ...
      if (progress === 80) {
        setLogs(prev => [...prev, tamperState 
          ? 'SYS - SHIELD PROTOCOL INITIATING QUARANTINE' 
      ...
      if (progress >= 100) {
        ...
        setLogs(prev => [
          ...prev, 
          tamperState 
            ? 'INTEGRITY FAIL - SECURE VERIFICATION FLAGGED TAMPERED DOCUMENT'
  ...
  return (
    <div className="w-full relative glass-panel rounded-xl overflow-hidden shadow-2xl border border-cyber-border/30 bg-black/30 backdrop-blur-md">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-4 py-3 bg-[#050b08] border-b border-cyber-border/30">
        <div className="flex items-center gap-2">
          ...
          <span className="font-mono text-[10px] text-cyber-text tracking-wider ml-2">CORE LEDGER SYSTEM v4.0.2</span>
        </div>
      ...
      <div className="p-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 flex flex-col gap-5">
          <div className="text-[10px] sm:text-xs font-mono text-neon-green/80 flex items-center justify-between">
            <span>CREDENTIAL INTEGRITY INDEX</span>
            <span>SHIELD V4.9</span>
          </div>
            ...
            <div className="relative flex items-start gap-4">
              ...
              <div className="flex-1 font-mono min-w-0">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0">
                  <h4 className="text-sm font-semibold tracking-wide text-white truncate">DEGREE HOLDER - RAHUL SHARMA</h4>
                  <span className={`text-[10px] px-2 py-0.5 rounded border self-start sm:self-auto shrink-0 ${
                    isTampered ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-neon-green/10 text-neon-green border-neon-green/20'
                  }`}>
                    {isTampered ? 'FAILED' : 'LEDGER OK'}
                  </span>
                </div>
                <p className="text-[11px] text-cyber-text mt-1 truncate">INSTITUTION: INDIAN INSTITUTE OF TECHNOLOGY (IIT)</p>
                <p className="text-[10px] text-cyber-text/70 mt-0.5 truncate">MAJOR: B.TECH COMPUTER SCIENCE & ENGINEERING</p>
                <div className="mt-4 pt-3 border-t border-cyber-border/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-0">
                  <div className="min-w-0 w-full">
                    <p className="text-[9px] text-cyber-text/60">BLOCKCHAIN MUTATION HASH</p>
                    <p className={`text-[10px] truncate w-full font-mono ${isTampered ? 'text-red-400' : 'text-neon-emerald'}`}>
                      {isTampered ? '0xERR_CORRUPT_SIGNATURE_MISMATCH' : '0x8f3c5b9e02c1103f6789e81b2a95c81d2f09a'}
                    </p>
                  </div>
                  <div className="flex flex-col items-start sm:items-end">
                    <p className="text-[9px] text-cyber-text/60">STAMP VALIDATION</p>
          ...
          <div className="flex flex-col sm:grid sm:grid-cols-2 gap-3 mt-4 sm:mt-1 relative">
            {/* Click Indicator */}
            {!animating && status === 'VERIFIED' && !isTampered && (
              <div className="absolute -top-9 sm:-top-10 left-1/2 sm:left-[25%] -translate-x-1/2 flex flex-col items-center animate-bounce pointer-events-none z-20">
        ...
        <div className="lg:col-span-5 flex flex-col gap-4">
          <div className="text-xs font-mono text-cyan-400 flex items-center justify-between">
            <span>P2P BLOCKCHAIN NODES</span>
            <span className="animate-pulse">ACTIVE PEERS: 8</span>
          </div>
          ...
          <div className="flex justify-between items-center bg-[#050b08] p-3 rounded-lg border border-cyber-border/40 relative overflow-hidden">
            ...
            <div className="flex flex-col items-center">
              ...
              <span className="text-[9px] font-mono text-cyber-text mt-1">NODE 01</span>
            </div>
            ...
            <div className="flex flex-col items-center">
              ...
              <span className="text-[9px] font-mono text-cyber-text mt-1">NODE 02</span>
            </div>
            <Link className={`w-4 h-4 text-cyber-border ${animating ? 'animate-pulse text-neon-green' : ''}`} />
            <div className="flex flex-col items-center">
              <div className={`p-2 rounded-full ${isTampered ? 'bg-red-500/10 text-red-500 border border-red-500/30' : 'bg-neon-green/10 text-neon-green border border-neon-green/30'} ${animating ? 'animate-pulse' : ''}`}>
              ...
              <span className="text-[9px] font-mono text-cyber-text mt-1">NODE 03</span>
            </div>
          </div>
            ...
            >
              {logs.map((log, index) => (
                <div key={index} className={`leading-relaxed border-l-2 pl-2 ${
                  log.includes('CRITICAL') || log.includes('INTEGRITY FAIL')
